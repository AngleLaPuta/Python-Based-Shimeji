...
def get_windows():
    ...
    def callback(hwnd, ctx):
        if hwnd == shimeji_hwnd or not win32gui.IsWindowVisible(hwnd):return
        if win32gui.IsIconic(hwnd):return
        rect = win32gui.GetWindowRect(hwnd)
        style = win32gui.GetWindowLong(hwnd, win32con.GWL_STYLE)
        ex_style = win32gui.GetWindowLong(hwnd, win32con.GWL_EXSTYLE)
        if (rect[2] - rect[0] == 0 or rect[3] - rect[1] == 0 or ex_style & win32con.WS_EX_TOOLWINDOW or style & win32con.WS_MINIMIZE):return
        ...
        if root_target != root_current:return
        windows.append(rect)
...
def check_collision():
    ...
    if closest_platform:
        #y = closest_platform[1] - char_height
        velocity_y = 0
        grounded = True
        return True
...
def update_animation():
    ...
    if state == 'dangle legs':window.geometry(f"+{int(x)}+{int(y)+15}")
    else:window.geometry(f"+{int(x)}+{int(y)}")
...
def update():
    ...
    if state == 'walk' and grounded:
        ...
        x += 5 * direction
        if random.randint(0, 100) == 72: state = 'fall'

    elif state in ['sit','sit up','dangle legs','lay down','lie down swing legs']:
        if random.randint(0,100)==72: state ='walk'
    elif not grounded:
        state = 'fall'
